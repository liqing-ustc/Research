...
> - **Rating**: 3 - Foundation（用 "timestep ≡ mask" 把 policy/forward/inverse dynamics/video prediction 统一到单一 diffusion 网络，并给 action-free video cotraining 一个干净架构，是 unified video-action diffusion 路线的 reference 工作）
...
**分数**：3 - Foundation
**理由**：UWM 的核心贡献——"diffusion timestep ≡ soft mask" 这一观察让 policy / forward dynamics / inverse dynamics / video prediction 在单一 DiT 内统一，且天然支持 action-free video cotraining——是 unified video-action diffusion 这条路线的 reference formulation。Strengths 里强调的 "simple, scalable, generalizable" 以及对 PAD 的清晰分离（Table I 中 PAD 在 Block-Cabinet/Rice-Cooker 0% vs UWM 0.76/0.60）、对 "pure aux loss 解释" 的 Table VIII 反驳，都是方向必读级 insight。与 Frontier 档（2）的差异在于：它不是又一个 SOTA baseline，而是提供了"mask as continuous timestep"这一可泛化到后续 unified diffusion / Diffusion Forcing 一类工作的 conceptual lens；RSS 2025 接收 + DROID 平台上的 real-robot 验证也支持社区影响力判断。未达到 ImageNet / DROID 那种 dataset-level foundation，但在 video-action unified diffusion 这一子方向内是绕不开的。
