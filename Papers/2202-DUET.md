...
> - **Rating**: 3 - Foundation（VLN 领域 de facto baseline，范式级跃升 +22% SR，后续工作几乎必引必比）
...
![](assets/DUET/method_compare.png)
...
**理由**：按 field-centric rubric 属于 Building Block——在 VLN domain 内，DUET 是"map-as-action-space + dual-scale"范式的奠基工作，REVERIE +22% SR 的跨度（Strengths#1）以及在 R2R / SOON / REVERIE 三个 benchmark 同时 SOTA 决定了它是后续工作的必引必比 baseline（Notes 已列出 LH-VLN / VLN-R1 / StreamVLN 等后继均延续其 topological-map 范式）。区别于 2 档：它不是某个窗口期的 SOTA 方法，而是已经成为 VLN 方法论本身的一部分——Google Scholar 显示已被 500+ 引用（截至 2026-04），且 Chen et al. 后续的 ScaleVLN / GridMM 等均以 DUET 为 backbone，范式尚未被取代。
